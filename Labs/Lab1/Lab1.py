...
def VLSR(Ro,mu=6.379,v_sun=12.24*u.km/u.s):
    ...
    Inputs: Ro (astropy units kpc) Distance from the Sun
                ...
                vsun (astropy units km/s) the peculiar motion of the Sun 
                    in the v diretion (Schonrich+2010)
    Outputs: VLSR (astropy units km/sec) The local standard of rest
    """
...
# Different values of the distance of the Galactic Center
...
def TorbSun(Ro, Vc):
    """ Function that computes the orbital period of the Sun
        T = 2 pi R / V
    Inputs: Ro (astropy quantity) distance to the Galactic Ceter from the Sun (kpc)
            Vc (astropy quantity) vleocity of the sun in the "v" direction (km/s)
...
Vsun = VLSR_Abuter + VsunPec # the total motion of the sun in the "v" dir
...
def massIso(r, VLSR):
    """ This function will copute the dark matter mass enclosed within a given distance, r, assuming
    an Isothermal Sphere Model
        M (r) = VLSR^2/G *r

    Inputs :    r (astropy quantity) distance from the Galatic Center (kpc)
...
Vleo = 196*u.km/u.s #Speed of the Leo I Sohn et al
r = 260*u.kpc
