...
for different snapshots to see how the percentage in the halo changes over time
...
# Compute COM of M31 using disk particles  (coordinates)
...
#Create a COM of the entire system
COMSYS = (COMPM31 + COMPMW)/2

# Determine positions of disk particles relative to COM for full system
...
# Save to a file
plt.savefig('ResearchAssignment3_CombinedHist.png')
#========================================================================
...
#Set up histogram particles for combined system
Hsys, xedges, yedges = np.histogram2d(xSYS, ySYS, bins=150, density=False)
#masked array not needed
#Hsm = np.ma.masked_where(Hsys == 0, Hsys)
X, Y = 0.5*(xedges[1:]+xedges[:-1]), 0.5*(yedges[1:]+yedges[:-1])
#Get count of M31 particles with same bins as combined system
Hm31, xedges, yedges = np.histogram2d(xDM31, yDM31, bins=(xedges, yedges),
                                      density=False)
# x_bin_sizes = (xedges[1:] - xedges[:-1]).reshape((1,nbins_x))
# y_bin_sizes = (yedges[1:] - yedges[:-1]).reshape((nbins_y,1))

#ratio of M31 particles to combined system particles
Grat = Hm31/Hsys

#make figure of system to make sure it looks right(check)
#fig, ax = plt.subplots(figsize=(12, 10))
#create plot with X,Y
...
#make cross-section  
#find particles less than zero in the z-direction for the combined system and the M31 dark matter
cutSysIdx = np.nonzero(zSYS<0)
cutDM31Idx = np.nonzero(zDM31<0)
fig, ax= plt.subplots(figsize=(12, 10))


#plot histogram of the system cross-section
...
#========================================================================
#========================================================================
#Create same color map of the cross-section
...
"""WHAT'S LEFT TO DO:
    I still need to modify this code to make a function so that I can easily loop through
    multiple snapshots. The end goal is to make a double graph showing the color percentage 
    above and below the z-axis. I want to make a series of these double graphs at different
    snapshots throughout the merger.
    
    Could potentially also use cross-section function to look at symmetry by making cross sections
    in all three coordinates(x-crosssection in YZ, y-crossection in XZ)
"""
